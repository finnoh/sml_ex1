...
To find the optimal penalty strength $\lambda$ and the weight of the elastic net $\alpha$, $K$-fold cross-validation is used. $K$-fold cross-validation starts with splitting the data set into $K$ folds. Subsequently, $K$ iterations are ran as follows: for each iteration $k \in \{1,\ldots,K\}$, fold $k$ is the test set, whereas the remaining $(K-1)$ folds together form the training set. On the training set, the elastic net is estimated for all $\lambda_{i} \in (10^{-6+i})_{i=1}^{10}$. Moreover, for each $\lambda_{i}$, all corresponding $\alpha \in \{0,0.01,\ldots,1\}$ are estimated, such that there are $1000$ combinations of the hyperparameters $\alpha$ and $\lambda$. All estimated elastic nets (i.e., all combinations of the hyperparameters), are used to predict the dependent variable in the $k$th fold. When this procedure has been completed for all $K$ folds, we have obtained fitted values for every observations in the data set, for all elastic nets. Consequently, prediction errors can be computed.
...
```{r mape}
plot_coef_rmse + labs(title = "MAPE Coefficients GLMNET vs. MM")
```

To explore this hypothesis, we investigate how the differences between the estimates develop for lower convergence thresholds $\epsilon$ of the MM algorithm. 

```{r}
plot_MAPE_eps + labs(title = TeX("Median MAPE for decreasing $ \\epsilon$"))
```
